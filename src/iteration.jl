    ...
    if argi isa Expr && argi.head === :macrocall && argi.args[1] == Symbol("@unwrap")
        ...
        if tmp.head === :for
            ...
            if iter_spec.head === Symbol("=")
                ...
                throw(error(
                    "invalid iterator; @iterated allows only iterators in the form `for i in 1:N`",
                ))
            end
        else
            throw(error("invalid body; @unwrap requires a for loop within its body"))
        end
        return argi, 1
    end
    return argi, 0
...
Unwrap `for` loops over `Container`s.

This is a macro to be used to decorate a `for` loop over a `Container` fields to _unwrap_ its 
content resulting in a non-allocating iteration. 
"""
macro unwrap(expr)
    niter = 0
...
Create an _unwrappable_ function. 
See [`@unwrap`](@ref) for details of how `Container` loops are efficiently treated.
"""
macro iterated(f)
